...
export class TripService {
  ...
  /** Get a single booking submitted by the current user. */
  getMyTripBookingById(tripId: number, bookingId: number): Observable<BookingResponse> {
    return this.http.get<BookingResponse>(`${this.baseUrl}/${tripId}/bookings/mine/${bookingId}`).pipe(
      map((booking) => this.normalizeBooking(booking))
    );
  }
